...
- **`vmdk.rs`** — VMDK flat: `VmdkFlatReader` (parses the ASCII descriptor, concatenates `FLAT`/`ZERO` extents from `monolithicFlat` and `twoGbMaxExtentFlat` layouts), `export_vmdk_flat` (writes a single-extent `monolithicFlat`: `<base>.vmdk` descriptor + `<base>-flat.vmdk` raw extent), and `open_flat_extent_for_edit` (resolves the single FLAT extent and returns it `R+W` for in-place edit; rejects multi-extent / ZERO / sparse layouts). Sparse VMDKs (`KDMV` magic) are detected and rejected with a precise message — sparse support lands in Phase 4.
