...
| Trident pack | — (raw, size-detected) | Yes | Yes | Trident T-80 / T-300 pack image (ContrAlto2 / dorado layout: `[dummy][header][label 10w][data 1024w]` per sector, little-endian, 2048-byte pages). The same Alto file system (TFS) on Trident hardware; recognized by the exact T-80 (~76 MB) / T-300 (~285 MB) size, surfaced as an `Alto BFS` volume. No period Trident pack survives, so validated by synthetic round-trip + the dorado constants. |
