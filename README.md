...
| TI-99/4A disk  | `.dsk`          | Yes            | No              | TI-99/4A (MiSTer TI-99_4A core) flat V9T9 sector image (SSSD 90K / DSSD / SSDD 180K / DSDD 360K). Read/browse/extract of the TI disk filesystem (VIB + FDIR + extent-based FDR files, big-endian); geometry read from the VIB. Content-detected via the "DSK" volume marker. |
...
| TI-99 | Yes | — | — (floppy, fixed geometry) | — | TI-99/4A (MiSTer TI-99_4A core). A proper little filesystem (unlike the contiguous CoCo/ZX floppies): a Volume Information Block with an allocation bitmap (bit set = used), a sorted File Descriptor Index of File Descriptor Records, and **extent-based** files (a packed 3-byte-per-cluster chain). All fields big-endian (TMS9900). Flat V9T9 `.dsk` (SSSD 90K / DSSD / SSDD 180K / DSDD 360K); geometry read from the VIB. Browse + extract; content-detected via the "DSK" volume marker. Cross-validated against MAME's `imgtool` reader and an independent clean-room oracle (`scripts/ti99-oracle.py`). Edit / create-blank / fsck to follow. |
