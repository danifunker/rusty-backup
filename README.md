...
| UCSD p-System  | Yes    | No   | —               | —    | UCSD Pascal (Apple II/III, PC, and many late-1970s/80s machines). A flat single directory of up to 77 **contiguous** files (no subdirectories, no allocation bitmap). Read-only browse; auto-detected via the block-2 volume label, with byte order recovered from the label's `DLASTBLK`. |
