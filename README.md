...
| SquashFS (v4.0) | Yes (read-only) | No | — | — | The read-only compressed filesystem Linux appliance and live images are built from — Buildroot roots, and `casper/filesystem.squashfs` inside practically every live-CD ISO. Browse + read files and symlinks, including multi-block files and fragment-packed tails. Compressors: gzip, XZ, LZMA, LZ4, and zstd — gzip is the one exercised by our real-image fixtures (Ubuntu live-CD roots); the rest are implemented but not yet fixture-tested. **LZO images are refused by name** rather than misparsed. Auto-detected by its `hsqs` magic as a superfloppy or inside an MBR type-0x83 partition. No edit or resize path by design: SquashFS is built offline by `mksquashfs` and has no in-place write story. |
