...
| Xerox Pilot/Cedar volume | `.pdi` (`fsFamily=2`) | Yes | Yes (create / add file) | D-machine Pilot/Cedar filesystem in a PARC Disk Image. Physical/logical volume roots, subvolume table, VAM, run-table files; both file-ID generations (32-bit Cedar nucleus / 80-bit original Pilot via `flags` bit 2). Surfaced as a read-only `Pilot/Cedar` volume in the GUI; blank-volume + add-file via `pilot_probe`. Defined from the Mesa sources (no period sample) — see `docs/` PARC specs. |
...
| Alto BFS | Yes | Yes | Yes (resize) | — | Xerox Alto Basic File System (the same on-disk format as the Alto OS file primitives and TFS, on Diablo 31/44 packs). Flat SysDir namespace, leader pages, page-chain files, and **out-of-band sector labels** (the file structure lives in the labels, not the data area). Browse + extract + add/delete + resize; opened from `.pdi` / `.bfs` / `.copydisk` / `.altodisk` / Salto `.dsk` packs (edits save as PDI). Validated against every CopyDisk pack in the CHM Xerox PARC archive and the Salto emulator's disks. TFS (Trident) planned. |
| Pilot / Cedar | Yes | No (read-only in GUI) | — | — | Xerox D-machine Pilot/Cedar nucleus filesystem (Dolphin/Dorado/Dandelion), structurally unrelated to BFS: physical/logical volume roots (seals `121212`₈ / `131313`₈), a subvolume table, the VAM free bitmap, and extent-based files with run tables — all behind **out-of-band sector labels**. Both file-ID generations (32-bit Cedar nucleus / 80-bit original Pilot). Browse + extract in the GUI; blank-volume creation + add/delete files + a real VAM (rebuilt from page labels) via the `pilot_probe` tool. Defined from the original Mesa sources + disk microcode (no surviving sample disk); validated by round-trip. See the PARC specs under `docs/`. |
