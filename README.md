...
| UCSD p-System  | Yes    | Yes  | —               | —    | UCSD Pascal (Apple II/III, PC, and many late-1970s/80s machines). A flat single directory of up to 77 **contiguous** files (no subdirectories, no allocation bitmap). Browse + edit (create / delete / rename — contiguous first-fit allocation; a create that won't fit any single free run is refused rather than fragmenting). Create blank volumes with `rb-cli new --fs ucsd` (alias `pascal`). Auto-detected via the block-2 volume label, with byte order recovered from the label's `DLASTBLK`. Writes validated against an independent clean-room `fsck` (`scripts/ucsd-oracle.py`). |
