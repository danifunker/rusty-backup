...
| ext2 / ext3 / ext4 | Yes | Yes | Yes             | Yes (check + repair on ext2/3; check-only on metadata_csum ext4) | Early Linux installs onward. Create blank volumes with `rb-cli new --fs ext` (plain ext2) or `--fs ext3` (ext2 plus an empty jbd2 journal); both `e2fsck`-clean. fsck reconciles the block + inode bitmaps and free counts against the computed allocation (Pass-5 style) and repairs them, validated against `e2fsck`. On `metadata_csum` volumes issues are reported but repair is withheld (crc recompute not yet implemented). |
...
| Apple DOS 3.3  | Yes    | Yes  | —               | —    | Apple II. 140 KB 5.25" disks (`.dsk` / `.do` / `.po`, sector-order auto-detected). Gated to the exact 140 KB geometry. |
...
| CP/M (2.2 / 3 / Plus) | Yes | Yes | — | — | Amstrad CPC / PCW, Tatung Einstein, Spectravideo SV-328, MITS Altair, Grant Searle MultiComp, ZX Spectrum +3. Nine built-in disk-parameter blocks (DPBs); CP/M has no on-disk signature, so the format is chosen explicitly (`--fs cpm:<preset>`). |
