...
| ext2 / ext3 / ext4 | Yes | Yes | Yes             | Yes (check + repair on ext2/3; check-only on metadata_csum ext4) | Early Linux installs onward. fsck reconciles the block + inode bitmaps and free counts against the computed allocation (Pass-5 style) and repairs them, validated against `e2fsck`. On `metadata_csum` volumes issues are reported but repair is withheld (crc recompute not yet implemented). |
