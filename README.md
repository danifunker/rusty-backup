...
| Acorn ADFS / FileCore | Yes | Yes (new-map) | — (fixed floppy geometry) | Yes (new-map) | Acorn Archimedes, BBC Master, RISC OS. Read + browse `.adf` floppy and bare / Arculator-wrapped `.hdf` HDD. Edit (create / delete / rename) on the **new-map** layouts (E / F / HD); D-format old-map stays read-only. Create blank E-format floppies with `rb-cli new --fs adfs` (800 KB, single-zone new-map FSM + empty Hugo root). fsck verifies the FSM zone checksums + cross-check and reconciles the allocated fragments against the directory-tree walk (dangling references, leaked fragments); repair re-stamps the zone checksums (a full FileCore map rebuild — RISC OS `*CheckMap` — is left for leaked-fragment reclamation). |
...
| **Archie** (Acorn Archimedes)  | ADFS / FileCore (new-map: read + edit + E-format create + fsck) | `.adf` floppy, bare + Arculator-wrapped `.hdf` HDD |
