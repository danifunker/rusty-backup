...
| MFS | **Yes** | Yes | **Yes** | — **complete**. Create: `rb-cli new --fs mfs` (Macintosh File System — Mac 128K / 512K / Plus; 400 KB / 800 KB floppy: zeroed boot blocks, MDB, all-free allocation map, empty directory; geometry solved to a fixed point under the 12-bit-map 4094-block ceiling). fsck: volume-allocation-map reconciliation against every in-use directory entry's data + resource fork chains (broken/cross-linked chains, leaked blocks, free-count + file-count); repair reclaims unreachable blocks and resyncs the MDB counters, withheld on structural damage (the MFS map encodes each file's chain linkage, so a broken chain could hide live blocks). |
