...
- [~] **Net 7b–7i — networked backup/restore** (the path to "both" local + net).
  **7a–7g done + qemu-verified — the core network feature is complete**: the
  ...
  - [x] 7h incremental backup *(detection + bootability flag + opt-in whole-disk skip /INCREMENTAL; per-partition skip = optional refinement)*
  - [~] 7i level-2 swap dealloc + desktop swap parity *(desktop parity + CHD swap DONE; level-2 dealloc deferred per §6c)*
