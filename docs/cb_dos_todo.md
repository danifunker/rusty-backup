...
- [~] **Net 7b–7i — networked backup/restore** (the path to "both" local + net).
  **7a–7g done + qemu-verified — the core network feature is complete**: the
  ...
  - [~] 7h incremental backup *(detection + bootability flag DONE; streaming-skip remains)*
  - [~] 7i level-2 swap dealloc + desktop swap parity *(desktop parity DONE; level-2 + CHD swap remain)*
