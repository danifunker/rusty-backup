...
| **B** Generalize FS verbs + globbing | **shipped** | `b627eca..f40ed4c` — ls/put/get/rm/mkdir/fsck route through `open_filesystem` / `open_editable_filesystem`, bash-equivalent globs (`*`, `?`, `[...]`, `**`, `{a,b}`) with `--include`/`--exclude` (exclude-wins). Blank creators `create_blank_fat` (FAT12/16, auto-select by size), `create_blank_efs` (IRIX, single CG), `create_blank_affs` (FFS/OFS variants) now live in `src/fs/{fat,efs,affs}.rs`; `rb-cli new --fs {fat,efs,affs}` is wired up. Superfloppy auto-detect handles EFS + AFFS magic. FAT32 formatting still deferred (vintage targets rarely need it; restore from a backup if needed). |
...
| **D** Batch + resize/expand cluster | **3 of 3 shipped (+1 deferred sub-item)** | `6a59121` — `rb-cli batch SCRIPT.json`, `batch-template`. `resize IMG@N --size BYTES` routes through `fs::resize_filesystem_for` (FAT/NTFS/exFAT/HFS/HFS+/ext/btrfs/SFS/PFS3/AFFS/EFS). `expand IMG@N --size --block-size --output` lifts the GUI's HFS expand-block-size dialog into `src/model/hfs_expand_runner.rs` and exposes it as a CLI. **Deferred sub-item only**: whole-volume `format` op + multi-partition `disk` block in batch (gated on FAT/EFS/AFFS blank creators from Phase B). |
...
| **H** Documentation | **4 of 5 shipped** | `4cca5dd` — auto-generated `docs/cli-reference.md` (via `cargo run --example generate_cli_reference`), hand-written `docs/cli-examples.md` cookbook-lite, README rewrite. Manpage gen example shipped in Phase A. `docs/cli-cookbook.md` (this slice) — six longform end-to-end recipes. **Deferred**: Windows HTML help (low-priority; the auto-reference + cookbook cover the same surface). |
