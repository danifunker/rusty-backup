...
- **Filesystems:** FAT12/16/32, exFAT, NTFS, HFS, HFS+/HFSX,
  ...
  ADFS (read + new-map edit + E-format create + new-map fsck),
...
| Archie | Acorn Archimedes | Floppy, HDD | ADFS / FileCore | **Yes** (new-map) — full quartet on the new-map layouts (E / F / HD): read + browse, edit (create / delete / rename via the FSM allocator), create blank E-format floppies (`rb-cli new --fs adfs`, 800 KB single-zone new-map FSM), and fsck (zone-checksum + cross-check verify + allocated-fragment-vs-directory-tree reconciliation; repair re-stamps the zone checksums, leaked-fragment reclaim withheld pending a full `*CheckMap`-style map rebuild). Disc Record scan spans the HD / E-format-floppy / legacy-floppy candidate offsets (0xFC0 / 0x404 / 0xDC0); byte-correct against marutan.net blank HD samples (`blank256E.hdf`, `blank1024Eplus.hdf`) and the 8bs.com `arc-04.800.adf` (E-format populated); `.adf` 800K floppy + bare and Arculator-wrapped `.hdf` HDD containers all handled. **Still open:** D-format (old-map) write, and double-sided `.dsd`. |
