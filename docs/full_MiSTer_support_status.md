...
| X68000 | Sharp X68000 | Floppy, SASI/SCSI HDD | Human68k (FAT-derived dialect) | **Yes** — floppy (Human68k read + Add/Delete via Sharp `.d88` container) and SASI/SCSI HDD (`.hda` / `.hdf` / `.hds`) read/browse/extract/add/delete/mkdir + in-place FS grow/shrink (`rb-cli resize`), incl. real BlueSCSI `X68SCSI1` 1024-byte-sector images (sector size derived from the boot signature; Sharp/KG big-endian BPB + big-endian FAT). Verified byte-exact on the BlueSCSI HD10 fixture across a grow→shrink round-trip (multi-cluster `COMMAND.X` + Japanese filenames survive). Restore-side partition-table patching is sector-size-aware; only the 256-byte-SASI partition-data write offset (`start_lba*512` vs the true byte offset) remains deferred — see [`OPEN-WORK.md`](OPEN-WORK.md) §8. |
