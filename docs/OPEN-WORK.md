...
| # | Tier | Deliverable |
|---|---|---|
| R.1 | A | Superblock parser at byte 65536 (`s_block_count`, `s_free_blocks`, `s_blocksize`, `s_bmap_nr`), magic dispatch (3.5/3.6, reject 4). Wire `probe_0x83_fs_type` + `fs_name_for("reiserfs")` + `open_filesystem_by_string`. Inspect tab shows ReiserFS + sizes. Port from `fs_reiser.cpp`. Unit test against a hand-built superblock; gated test against a real `mkfs.reiserfs` fixture (grab `reiserfsprogs` now while it's packaged). |
| R.2 | A | Load the bitmap blocks (first right after the superblock, then one every `blocksize * 8` blocks). Implement `compact_partition_reader_by_string` (stream allocated blocks) + `last_data_byte` so `partition_minimum_size` returns a real floor. `is_expensive_minimum` = false. Round-trip compaction test. |
| R.3 | B | S+tree walker → `read_file` + `list_directory` + `root`. Internal nodes → leaf nodes → item heads; parse stat-data (v1/v2), directory items (entry headers + name area), indirect/direct items for file bodies (incl. **tail packing** — small file tails packed into a direct item). Read-only. Multi-session; reference `debugreiserfs`. |
