...
| 5.5c | **GHO Fast-LZ decoder** | Norton Ghost | Standalone Fast-LZ block decoder (custom LZ77 variant: 4096-entry hash, 16-bit control words, min 3-byte match, hash `h = ((-24993 * (b2 ^ (16*(b1 ^ (16*b0))))) >> 4) & 0xFFF`). Tested against a real `0x02`-compressed block extracted from a corpus fixture. No decode-to-temp yet. | `github.com/nyarime/gho` (MIT). **Med.** |
