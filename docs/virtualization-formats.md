...
| 5.6-span | **GHO `.GHS` span chain** | Norton Ghost | Auto-discover + concatenate `.GHS` continuation files when the user picks any one file of a spanned set (three corpus naming patterns: `SECTO00N.GHS`, `disk-00N.GHS`, `name.GHO.NNN`). | `nyarime/gho/span.go`. **Med.** |
