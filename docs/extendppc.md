...
| **ext2 read** | Medium | Small spec, LE on-disk (needs byteswap macros on PPC). Inode + directory + indirect blocks. ~1500 LOC. |
| **exFAT read** | Medium | Bitmap + cluster heap, directory entry sets. ~1500 LOC. |
