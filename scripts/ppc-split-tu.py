...
`promoted.h` gets one `#define NAME NAME__rbsplit_<tu>` per promoted name and
each unit includes it first, so the definition and every reference move together
at no cost. The `<tu>` is a digest of the source file name, because a constant
suffix only separates a promoted name from the *owning crate's* copy: when two
crates in one link are both split - protobuf and the agent, say - each promotes
its own copy of the same `alloc` monomorphisation, and a shared suffix makes
those two collide with each other instead:
...
mrustc uses the same trick itself (`#define ...panic_fmt0g
...
# Per translation unit, not constant: see `rename_suffix`.
RENAME_SUFFIX = b"__rbsplit"


def rename_suffix(base):
    """The suffix for one translation unit's promoted names.

    A constant suffix is not enough. It keeps a promoted crate-local copy from
    colliding with the *owning* crate's global definition, which is what the
    rename was written for - but two split crates in the same link each promote
    their own copy of the same shared monomorphisation, and with one suffix
    those collide with each other. Keying it to the source file name separates
    them; the digest keeps it short and identifier-safe (the file name has `.`
    and `-` in it).
    """
    digest = hashlib.sha256(base.encode()).hexdigest()[:8].encode()
    return b"%s_%s" % (RENAME_SUFFIX, digest)
...
def split(source, outdir, units, force=False, quiet=False):
    ...
    # The suffix is in the stamp so that changing the renaming scheme
    # invalidates splits made by an older version of this script - the source
    # itself is unchanged, so nothing else here would notice.
    stamp = "%d %d %d %s" % (os.path.getmtime(source), os.path.getsize(source),
                             units, suffix.decode())
    ...
    # fight with it.
    #
    # The suffix is per translation unit (`rename_suffix`), because two split
    # crates in one link otherwise promote the same shared monomorphisation to
    # the same new name and collide with each other.
