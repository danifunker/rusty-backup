...
/// The gitignored per-host overlay written by `rb-regress oracles --detect`.
#[derive(Deserialize, Default)]
struct LocalOraclesFile {
    #[serde(default)]
...
impl Registry {
    pub fn load(regression_dir: &Path) -> Result<Registry, String> {
        ...
        // Machine-specific availability, layered over the tracked registry.
        // `oracles.toml` says what a tool is and what it proves — portable
        // knowledge. Whether *this* box has it, and where, is not portable and
        // is gitignored: absolute paths in a public repo were how a local
        // `D:/ROMs/...` ended up tracked. Generated by `rb-regress oracles
        // --detect`; a row here replaces the tracked one for the same
        // (oracle, platform).
        let local = data.join("oracles.local.toml");
        if local.is_file() {
            match fs::read_to_string(&local)
                .map_err(|e| e.to_string())
                .and_then(|t| toml::from_str::<LocalOraclesFile>(&t).map_err(|e| e.to_string()))
            {
                Ok(lf) => {
                    for a in lf.availability {
                        r.availability
                            .retain(|e| !(e.oracle == a.oracle && e.platform == a.platform));
                        r.availability.push(a);
                    }
                }
                Err(e) => r
                    .warnings
                    .push(format!("{}: {}", local.display(), e)),
            }
        }
