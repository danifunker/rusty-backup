...
fn fsck_reports_clean_on_real_fixture() {
    // `fsck` on the .atr exercises the container-decode read path (the same
    // one ls/get/inspect use). The real DOS 2.0S fixture must be clean; `run`
    // panics on a non-zero exit, so a clean run implies no fsck errors.
    let tmp = tempfile::tempdir().unwrap();
    let img = fixture_to(tmp.path());
